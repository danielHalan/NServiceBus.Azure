...
namespace Publisher
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var secondaryNamespaceManager = NamespaceManager.CreateFromConnectionString(ConnectionStrings.SecondaryNamespace);
            var primary = MessagingFactory.CreateFromConnectionString(ConnectionStrings.PrimaryNamespace);
            var secondary = MessagingFactory.CreateFromConnectionString(ConnectionStrings.SecondaryNamespace);

            var sendAvailabilityOptions = new SendAvailabilityPairedNamespaceOptions(secondaryNamespaceManager, secondary, 10, TimeSpan.FromSeconds(10), true);
            primary.RetryPolicy = new RetryExponential(TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(30), 10);
            primary.PairNamespaceAsync(sendAvailabilityOptions).Wait();

            //var client = primary.CreateQueueClient(id);
            ...
            while ("x" != key)
            {
                ...
                var partition = rand.Next(0, 1);

                var client = primary.CreateTopicClient("partition-" + partition);
